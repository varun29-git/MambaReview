...
USE_AMP = DEVICE == "cuda"  # Mixed precision only stable on CUDA
...
def generate(model, tokenizer, prompt: str, max_new_tokens: int = 100, temperature: float = 0.8):
    ...
    # Mamba-2 autoregressive fast generation using caches!
    caches = model.allocate_inference_cache(1, DEVICE)

    for step in range(max_new_tokens):
        # First step passes the entire prompt to prefill the cache
        if step == 0:
            logits = model(input_ids, caches)
        else:
            # Subsequent steps only pass the last generated token
            logits = model(input_ids[:, -1:], caches)
